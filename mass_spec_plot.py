...
def generate_massspectra_plot_automatic_labels(ms2_spec, labels_array, output_filename, min_x, max_x):  # ms2_spec=[(m/z,intensity)]
    # print("Generate {0} mass spectra plot ({1}.{2}) from {3}.[xy/txt]".format(filetype.upper(), output_filename,filetype, input_filename))
    ...
    plotheight_mm = plotwidth_mm * 0.25

    hspace_mm = 0.1  # horizontal space between two plots

    # read xy input data
    raw_values = pd.DataFrame(ms2_spec, columns=['m/z', 'intensity'], dtype=np.float64)
    ...
    # read txt captions
    caption_values = pd.DataFrame(labels_array, columns=["m/z", "caption"], dtype=np.float64)
    ...
    for i in caption_values.index:
        ...
        value, index = float(raw_values.loc[raw_values_index , "intensity_normalized"]), raw_values_index
    ...
    # plot spectra line
    ax.bar(raw_values["m/z"], raw_values["intensity_normalized"], color="black", linewidth=0.8)
    ...
    if len(caption_values.index) > 0:
        ...
        if len(list_text_pos) > 0 and max(list_text_pos) >= length_x_axis_pixel:
            print ms2_spec, labels_array, max_x,min_x
            ...
            for index in range(len(list_text_pos) - 1, 0, -1):
                ...
                if index > 1 and list_text_pos[index] - calc_delta > list_text_pos[
                            index - 1] + delta_x_text_labels_pixel:
...
if __name__ == "__main__":
    ms2_spec=[[51.7668, 1149.0], [57.3179, 1124.0], [58.4663, 1132.0], [63.6634, 1092.0], [64.1654, 1169.0], [69.6296, 2021.0], [72.0374, 1123.0], [85.9279, 1139.0], [96.9899, 1746.0], [146.5484, 1102.0], [148.6777, 1155.0], [168.8881, 1126.0], [195.0659, 1794.0], [244.558, 1201.0], [264.9502, 1116.0], [265.1452, 1710.0], [289.608, 1311.0], [333.2955, 1456.0], [342.878, 1698.0], [344.5818, 1335.0], [353.7041, 1308.0], [360.889, 1545.0], [361.2019, 1702.0], [361.2388, 1825.0]]
    labels= [[361.2388, ''], [361.2019, ''], [360.889, ''], [353.7041, ''], [344.5818, ''], [342.878, ''], [333.2955, ''], [289.608, ''], [265.1452, ''], [264.9502, ''], [244.558, ''], [195.0659, ''], [168.8881, ''], [148.6777, ''], [146.5484, ''], [96.9899, ''], [85.9279, ''], [72.0374, ''], [69.6296, ''], [64.1654, ''], [63.6634, ''], [58.4663, ''], [57.3179, ''], [51.7668, '']]
    generate_massspectra_plot_automatic_labels(ms2_spec,labels,'I:\Data_Analysis\160419_Sagh_SUB03948\Neg/ms2/10104220314',0,411.2756)
